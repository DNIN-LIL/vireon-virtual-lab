...
def run(cfg=None, medium: Medium | None = None, *args, **kwargs):
    ...
    wave_list = _to_str_list(cfg.get("waveforms")) or ["sine", "square", "triangle"]
    if "all" in wave_list:
    ...
    # Per-wave CSVs and metrics
    metrics = []
    overlay = []

    for mode in wave_list:
        y = A * _drive(mode, w * t)
    ...
    # Overlay plot (first few cycles for readability if sampling permits)
    fig, ax = plt.subplots()
    max_plot_points = 5000
    for mode, tt, yy in overlay:
        if tt.size > max_plot_points:
            # show about first N points to keep file light
            cut = max_plot_points
    ...
    ax.set_title("Waveform Shape Response (overlay)")
    ax.grid(True)
    ax.legend()
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, "waveform_overlay.png"))
    plt.close(fig)

    # Bundle useful arrays
    np.savez(
