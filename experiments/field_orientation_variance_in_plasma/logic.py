...
def run(cfg: dict, medium: Medium):
    # Parameters (mirror vacuum; only medium differs)
    ...
    # Frequency-dependent plasma screening
    # Option A (explicit): ask medium per-frequency
    scale = medium.electric_scale_at_freq(f_hz) if medium.is_plasma() else 1.0
    # Option B (implicit): set drive and call medium.electric_scale()
    # if medium.is_plasma():
    #     medium.drive_frequency = 2.0 * math.pi * f_hz
    #     scale = medium.electric_scale()
    ...
    for idx, t in enumerate(t_arr):
        ...
        for i, src in enumerate(sources):
            ...
            F = compute_force(
                ...
                waveform_func=sine,
                theta_deg=0.0,
                medium_scale=scale  # plasma screening applied
            )
            F_total += F
