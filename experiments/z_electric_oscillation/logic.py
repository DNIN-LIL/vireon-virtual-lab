...
# Sub-experiments under this package
SUBEXPS = {
    "vacuum": ["vacuum_sine", "vacuum_square", "vacuum_triangle"],
    "plasma": ["plasma_sine", "plasma_square", "plasma_triangle"],
}

# Absolute fallback package name if __package__ is empty
PKG_FALLBACK = "experiments.z_electric_oscillation"
...
def _import_child_abs(pkg_base: str, child_name: str):
    # e.g. experiments.z_electric_oscillation.vacuum_sine.logic
...
def run(cfg=None, medium: Medium | None = None, *args, **kwargs):
    """
    Dispatcher for z_electric_oscillation.
    Selects sub-experiments by medium (vacuum_* or plasma_*), forwards overrides,
    and injects __child_name__ so shared_logic can infer output paths.
    """
    if medium is None:
    ...
    for name in child_names:
        ...
        # Import child module (absolute first, then file fallback)
        try:
        ...
        # Load and override config; inject child hint for shared_logic
