...
def run(cfg=None, medium: Medium | None = None, *args, **kwargs):
    ...
    angles_deg = _to_float_list(cfg.get("orientation_angles_deg"), "orientation_angles_deg")

    mass = _as_float(cfg.get("mass"), "mass", 1.0)
    k = _as_float(cfg.get("default_k"), "default_k", 1.0)
    field_strength = _as_float(cfg.get("field_strength"), "field_strength", 1.0)
    ...
    for ai, deg in enumerate(angles_deg):
        ...
        for i, Q in enumerate(charges):
            ...
            for j, f_hz in enumerate(freqs_hz):
                forces[ai, i, j] = k * Qi * float(f_hz) * mass * field_strength * proj

        csv_path = os.path.join(out_dir, f"force_matrix_angle_{int(round(float(deg)))}deg.csv")
        np.savetxt(csv_path, forces[ai], delimiter=",", fmt="%.6e")
        ...
        ax.set_title(f"Force Magnitude (N) at {deg}°")
        fig.colorbar(im, ax=ax)
        fig.tight_layout()
        fig.savefig(os.path.join(out_dir, f"force_matrix_angle_{int(round(float(deg)))}deg.png"))
        plt.close(fig)
