...
def run_magnetic(cfg=None, medium: Medium | None = None, *args, **kwargs):
    cfg, medium = normalize_args(cfg, medium, list(args))
    if medium is None:
        raise ValueError("medium is required")

    # Magnetic requires 'magnetic_charge' and forbids 'charge'
    if _has_key(cfg, "charge"):
        raise ValueError("Invalid parameter for magnetic oscillation: 'charge' detected. Use 'magnetic_charge'.")
    if not _has_key(cfg, "magnetic_charge"):
        raise ValueError("Missing required parameter 'magnetic_charge' for magnetic oscillation.")
    ...
    g = as_float(cfg.get("magnetic_charge"), 1e-6)
    ...
    default_out = Path("output") / "z_magnetic_oscillation" / child_name
    ...
    # Counterclockwise: My leads Mx by 90 degrees
    Mx = A * drive(mode, phi)
    My = A * drive(mode, phi + 0.5 * math.pi)

    # Legacy scalar output (My)
    y = My
